<template>
  <div class="card" style="max-width: 400px; margin: 0 auto">
    ...
    <div v-if="message" :class="message.includes('Failed') ? 'error-message' : 'success-message'">
      {{ message }}
    </div>
    ...
    <form @submit.prevent="handleEmailAuth">
      ...
      <p style="text-align: center; margin-top: 1rem">
        ...
        <button 
          ...
          style="background: none; border: none; color: #42b883; cursor: pointer; text-decoration: underline"
          :disabled="isLoading"
        >
...
const handleEmailAuth = async () => {
  try {
    ...
    message.value = isSignUp.value ? 'Failed to create account' : 'Failed to sign in'
  } finally {
    isLoading.value = false
  }
}
