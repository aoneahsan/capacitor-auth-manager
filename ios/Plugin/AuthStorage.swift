...
class AuthStorage {
    ...
    func saveCredential(_ credential: AuthCredential, for provider: AuthProvider) {
        ...
        let filteredData = credentialData.compactMapValues { $0 }

        if let data = try? JSONSerialization.data(withJSONObject: filteredData),
           let jsonString = String(data: data, encoding: .utf8) {
            set("credential_\(provider.rawValue)", value: jsonString)
        }
    }

    func getCredential(for provider: AuthProvider) -> AuthCredential? {
        guard let jsonString = get("credential_\(provider.rawValue)"),
              let data = jsonString.data(using: .utf8),
        ...
        return AuthCredential(
            providerId: dict["providerId"] as? String ?? provider.rawValue,
            signInMethod: dict["signInMethod"] as? String ?? "oauth",
            ...
            rawNonce: dict["rawNonce"] as? String
        )
    }
