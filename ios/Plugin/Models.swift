...
struct AuthCredential {
    ...
    let scope: String?
    let rawNonce: String?
    // Server auth (offline-access) code — only populated by providers that support it
    // (e.g. Google native sign-in when a server client id is configured).
    let serverAuthCode: String?

    // Explicit memberwise-style initializer. Every optional defaults to nil so that
    // `serverAuthCode` can be added without changing any existing call site that passes
    // the original nine arguments in order.
    init(
        providerId: String,
        signInMethod: String,
    ...
    func toJSObject() -> JSObject {
        ...
        if let serverAuthCode = serverAuthCode { result["serverAuthCode"] = serverAuthCode }
        if let refreshToken = refreshToken { result["refreshToken"] = refreshToken }
...
enum AuthManagerError: LocalizedError {
    ...
    case operationNotSupportedOnPlatform(String, String)
    ...
    var errorDescription: String? {
        ...
        case .operationNotSupportedOnPlatform(let operation, let platform):
            return "\(operation) is not supported on \(platform). This method requires web-based providers. See docs/api-reference/CAPABILITY_MATRIX.md"
        }
    }
}