...
export interface GoogleAuthOptions {
  /**
   * OAuth 2.0 Web client ID. Required on web (GIS) and used as the Credential-Manager `serverClientId`
   * on Android when `serverClientId` is not set separately. This is the value you read the idToken's
   * `aud` against — the same Web client id you register for Firebase Google sign-in.
   */
  clientId: string;
  clientSecret?: string;
  scopes?: string[];
  hostedDomain?: string;
  /**
   * Web (server) OAuth client ID. **Required on Android** (Credential Manager) to receive an idToken,
   * and used on iOS to request a `serverAuthCode` for offline access. Falls back to `clientId` when omitted.
   */
  serverClientId?: string;
  /** iOS OAuth client ID (the `GIDClientID`). Falls back to the Info.plist `GIDClientID` when omitted. */
  iosClientId?: string;
  ...
  /**
   * Android Credential Manager: only surface accounts that have previously authorized this app
   * (silent / returning-user UX). Default false so first-time sign-in shows all Google accounts.
   */
  filterByAuthorizedAccounts?: boolean;
  /** Android Credential Manager: auto-select a single previously-used account without a tap. Default false. */
...
export interface GitHubAuthOptions {
  ...
  state?: string;
  /**
   * URL of a backend endpoint that exchanges the GitHub authorization `code` for an access
   * token. GitHub does NOT allow browser-side code→token exchange (it requires the client
   * secret and blocks CORS), so a server-side proxy is required to complete sign-in. The
   * proxy receives `{ code, redirectUri, state }` (POST, JSON) and must return GitHub's
   * token JSON (`{ access_token, token_type, scope, ... }`). Alias: `tokenEndpoint`.
   * @see https://docs.github.com/apps/oauth-apps/building-oauth-apps/authorizing-oauth-apps
   */
  tokenExchangeProxy?: string;
  /** Alias for {@link tokenExchangeProxy}. */
...
export interface FirebaseAuthOptions {
  ...
  /**
   * Default Firebase sign-in sub-method used when `signIn` is called without an explicit
   * method and no email/password credentials are supplied. Defaults to `'google'`.
   */
  defaultMethod?:
    | 'google'
    | 'facebook'
...
export interface AuthCredential {
  ...
  rawNonce?: string;
  /**
   * One-time server auth code (Google offline access). Exchange it on YOUR backend for a refresh
   * token / long-lived access token — never in the browser. Populated on iOS (and Android via the
   * Authorization API) when `serverClientId` + offline access are requested; not returned by the
   * web GIS id-token flow.
   */
...
export enum AuthErrorCode {
  ...
  // --- Added in 2.2.0 (additive, non-breaking) so standalone providers (sms / email-code /
  // magic-link / biometric / email-password / username-password / phone-password) throw enum
  // codes instead of bare string literals. Consumers can now switch on `error.code` reliably.
  CREDENTIALS_REQUIRED = 'auth/credentials-required',
  EMAIL_REQUIRED = 'auth/email-required',
  PHONE_REQUIRED = 'auth/phone-required',
  USERNAME_REQUIRED = 'auth/username-required',
  INVALID_USERNAME = 'auth/invalid-username',
  USERNAME_ALREADY_EXISTS = 'auth/username-already-exists',
  INVALID_CODE = 'auth/invalid-code',
  CODE_EXPIRED = 'auth/code-expired',
  INVALID_TOKEN = 'auth/invalid-token',
  NO_PENDING_VERIFICATION = 'auth/no-pending-verification',
  TOO_MANY_ATTEMPTS = 'auth/too-many-attempts',
  RESEND_DELAY = 'auth/resend-delay',
  SEND_CODE_FAILED = 'auth/send-code-failed',
  SEND_LINK_FAILED = 'auth/send-link-failed',
  VERIFICATION_FAILED = 'auth/verification-failed',
  SIGN_UP_FAILED = 'auth/sign-up-failed',
  SIGN_UP_DISABLED = 'auth/sign-up-disabled',
  ...
  NO_STORED_CREDENTIALS = 'auth/no-stored-credentials',
  STORAGE_ERROR = 'auth/storage-error',
  BIOMETRIC_LOCKOUT = 'auth/biometric-lockout',
  PROVIDER_UNSUPPORTED_OPERATION = 'auth/provider-unsupported-operation',
  // The active provider does not implement an optional capability (e.g. token refresh).
  // Same string the manager previously threw as a bare literal — now an enum member so
  // consumers can switch on `AuthErrorCode.OPERATION_NOT_SUPPORTED` reliably.
  OPERATION_NOT_SUPPORTED = 'auth/operation-not-supported',
  // The provider is recognized but not yet enabled in this release. As of 2.4.1 the package ships
  // a hardened Google provider and re-enables the remaining providers one at a time. Switch on this
  // to detect a "coming soon" provider distinctly from an unknown one.
  PROVIDER_NOT_ENABLED = 'auth/provider-not-enabled',
}
