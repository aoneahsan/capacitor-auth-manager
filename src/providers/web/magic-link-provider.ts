...
export class MagicLinkProvider extends BaseAuthProvider {
  ...
  // Persisted (SSR-guarded localStorage) so a magic link clicked from an email — which opens a
  // FRESH page load with empty in-memory state — can still find its pending entry to verify
  // against. Keyed by the link token (already present in the callback URL); we store only the
  // email + expiry, never any additional secret (see PendingVerificationStore + F-11).
  private pendingVerification = new PendingVerificationStore('magic-link');

  constructor(input: BaseProviderConfig | MagicLinkConfig) {
    super(resolveProviderConfig(AuthProvider.EMAIL_MAGIC_LINK, input));

    // Magic link uses its config as-is (no defaults to merge).
    this.config = this.options as MagicLinkConfig;
  ...
  async signIn(options?: MagicLinkOptions): Promise<AuthResult> {
    ...
    try {
      ...
      // Store pending verification (persisted; email + expiry only, keyed by the link token).
      ...
      if (!response.ok) {
        throw new AuthError(
          AuthErrorCode.SEND_LINK_FAILED,
      ...
      throw new AuthError(
        AuthErrorCode.SEND_LINK_FAILED,
        (error as ErrorLike).message || 'Failed to send magic link'
      );
    }
  }
  ...
  async verifyMagicLink(token: string): Promise<AuthResult> {
    ...
    if (Date.now() > pending.expires) {
      ...
      throw new AuthError(AuthErrorCode.TOKEN_EXPIRED, 'Magic link has expired');
    }

    // The persisted record stores the email under `identifier`.
    const email = pending.identifier;
    ...
    try {
      ...
      if (this.config.verifyUrl) {
        ...
        const user: AuthUser = {
          uid: data.uid || this.generateUid(email),
          email: email,
      ...
      throw new AuthError(
        AuthErrorCode.VERIFICATION_FAILED,
        (error as ErrorLike).message || 'Failed to verify magic link'
      );
    }
  }
  ...
  async revokeAccess(_token?: string): Promise<void> {
    // Magic link doesn't have persistent access to revoke; clear any pending verification.
    this.pendingVerification.clear();
  }
}
