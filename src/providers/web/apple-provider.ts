...
export class AppleAuthProviderWeb extends OAuthProvider {
  ...
  async signIn(options?: SignInOptions): Promise<AuthResult> {
    ...
    try {
      ...
      // Parse user information from response (validates ID token nonce + expiry first)
      const user = this.createAppleAuthUser(response, nonce);
  ...
  async refreshToken(_options?: RefreshTokenOptions): Promise<AuthResult> {
    // Apple's JS (web) Sign In flow never issues a refresh token, so the inherited
    // OAuthProvider.refreshToken() would fail with a confusing "No refresh token available".
    // Surface an explicit, accurate error instead: callers must re-authenticate.
    throw new AuthError(
      AuthErrorCode.OPERATION_NOT_ALLOWED,
      'Apple web sign-in does not issue refresh tokens; re-authenticate instead.',
      this.provider
    );
  }
  ...
  private createAppleAuthUser(
    ...
    // Validate the ID token's nonce (replay protection) and expiry BEFORE trusting any
    // claims. The signature is NOT verified client-side — always re-validate on your backend.
    // Defensive: only enforce the nonce when both an expected nonce and a token nonce exist.
    let claims: {
      sub?: string;
      email?: string;
      ...
      if (payload) {
        ...
        if (
          ...
          throw new AuthError(
            ...
            'Apple ID token nonce does not match the expected value',
            this.provider
          );
        }
        ...
        if (typeof claims.exp === 'number' && claims.exp * 1000 < Date.now()) {
          throw new AuthError(
            ...
            'Apple ID token is expired',
            this.provider
          );
        }
      } else {
        this.logger.error('Failed to decode Apple ID token payload');
      }
    }
