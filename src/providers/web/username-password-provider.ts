...
export class UsernamePasswordProvider extends BaseAuthProvider {
  get name(): string {
  ...
  // so username/password sessions stay in-memory exactly as before (no new storage persistence).
  private authToken: string | null = null;
  private refreshTokenValue: string | null = null;

  constructor(input: BaseProviderConfig | UsernamePasswordConfig) {
    super(resolveProviderConfig(AuthProvider.USERNAME_PASSWORD, input));
    const config = this.options as UsernamePasswordConfig;
    this.config = {
      allowSignUp: true,
      usernameRequirements: {
        minLength: 3,
  ...
  async signIn(options?: UsernamePasswordSignInOptions): Promise<AuthResult> {
    ...
    try {
      ...
      throw new AuthError(
        AuthErrorCode.SIGN_IN_FAILED,
        getErrorMessage(error) || 'Failed to sign in with username and password'
      );
    }
  }
  ...
  async signUp(options?: UsernamePasswordSignUpOptions): Promise<AuthResult> {
    ...
    if (usernameError) {
      throw new AuthError(AuthErrorCode.INVALID_USERNAME, usernameError);
    }
